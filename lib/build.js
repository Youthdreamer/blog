...
const { renderPost, renderIndexPage, renderPage, renderTagsIndex, renderTagPage } = require('./templates');
...
function build() {
  ...
  posts.forEach((p, i) => {
    fs.writeFileSync(
      ...
      renderPost(p, i, posts.length, posts[i - 1] || null, posts[i + 1] || null)
    );
  });
  ...
  // 标签索引 + 各标签页
  fs.writeFileSync(path.join(PATHS.out, 'tags.html'), renderTagsIndex(tagList));
