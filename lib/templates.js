...
const { SITE, PATHS, absUrl, PAGE_SIZE, VERSION } = require('./config');
...
function inlineCss() {
  const files = ['css/base.css', 'css/layout.css', 'css/components.css', 'css/effects.css'];
...
function head(title, desc, opts = {}) {
  ...
  const css = opts.inline
    ? inlineCss()
    : CSS_FILES.map((f) => `  <link rel="stylesheet" href="${prefix}assets/${f}?v=${VERSION}">`).join('\n');
  const js = opts.inline
    ? '' // 核心 JS 内联到 body 末尾（见 renderPage 的 404 分支）
    : JS_FILES.map((f) => `  <script defer src="${prefix}assets/${f}?v=${VERSION}"></script>`).join('\n');
...
${inlineCoreJs()}
${LINK_FIXER}`;
    return shell(title, body, { current: 'index', noindex: true, inline: true });
  }
  return '';
}
